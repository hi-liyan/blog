...
synchronized 在 jdk 1.6 之前使用重量级锁，使用的是操作系统提供的管程（Monitor）实现的，所以即便在没有线程竞争锁的情况下也可能会发生用户态与内核态（操作系统执行的代码在内核态）的频繁切换，造成不必要的性能损耗；
...
当存在锁竞争时，锁会从升级为轻量级锁，在获取轻量级锁时同样使用 CAS 尝试将轻量级锁的指针指向当前线程锁记录，如果指针修改成功，拿到轻量级锁，如果没有修改成功，则通过自旋继续通过 CAS 尝试修改锁的指针。当自旋一定次数仍拿不到锁，说明线程竞争比较严重，轻量级锁膨胀到重量级锁。
...
<SvgBox src="https://images.shiguangping.com//imgs/java-synchronized-Page-2.drawio.svg" />

偏向锁和轻量级锁都是采用了 CAS 获取锁，
