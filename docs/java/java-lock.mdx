...
<ImgBox src="https://images.shiguangping.com//imgs/20211207224920.png" width="100%" />
...
<ImgBox src="https://images.shiguangping.com//imgs/20211207234758.png" width="100%" />

如果只有一个线程访问同步代码，那么每次获取锁都需要系统调用操作系统的管程（Monitor），用户态与内核态的频繁切换会造成大量系统开销，这显得有些小题大做了。Java 的大佬们发现，在大多数情况下，锁不仅不存在线程竞争，而且总是由一个线程多次获取，为了让线程获得锁的代价更低，大佬们在 Jdk 1.6 时为 synchronized 引入了偏向锁。
...
<ImgBox src="https://images.shiguangping.com/imgs/20211208112008.png" width="100%" />

偏向锁是考虑到在没有锁竞争的情况下，偏向锁会当前持有锁的线程，不需重复获取锁。当有线程竞争情况发生时，例如线程A持有偏向锁（偏向锁在没有竞争时不会主动释放）,此时线程B也要执行同步代码，线程B会通过 CAS 操作尝试去替换偏向锁的 thread_id 为自己的 thread_id，因为当前偏向锁被线程A持有，所以导致替换失败，接着会进入撤销偏向锁的流程。

虚拟机会等待偏向锁的线程到达全局安全点（safe point），全局安全点是虚拟机用来记录一些信息的位置，此时线程A会被暂停，接着检查线程A是否执行完了同步代码：

如果已经执行完了同步代码，则偏向锁会被撤销，锁对象的 Mark Word 会将锁标志为置为 01，biased_lock（是否是偏向锁）被值为 0 ，线程A被唤醒，然后线程A、B会重新通过 CAS 获取偏向锁。（这两个线程谁都有可能再次拿到偏向锁）

如果线程A没有执行完同步代码，锁会被升级成轻量级锁，


<SvgBox src="https://images.shiguangping.com/imgs/%E5%81%8F%E5%90%91%E9%94%81%E5%8D%87%E7%BA%A7%E8%BD%BB%E9%87%8F%E7%BA%A7%E9%94%81.svg" width="100%" />
