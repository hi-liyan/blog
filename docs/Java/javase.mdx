...
一个类通常是由 **成员属性**、**构造方法**、**成员方法** 组成。属性用来定义这个类的状态，也就是这个类要存储的数据；构造方法用来构造类的实例对象；成员方法提供该类的行为。
...
**类** 是对一类具有相同属性和行为事物的抽象，类的具体实例称之为 **对象**，它是真正表示具体的事物。例如用 **Cat** 这个类表示猫，假如有一只猫叫"花花"，那这个"花花"就是 Cat 类的对象。
